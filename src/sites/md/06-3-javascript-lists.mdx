...
Wenn der zweite Eintrag in der Liste `false` ist, wissen
...
Das was wir gerade implementiert haben, ist die `split()`
-Funktion in Javascript. Diese ist wie `join()` eine
...
Lesen Sie im Internet nach was die `split()`-Funktion in
Javascript macht. Erstellen Sie eine zweite Version von dem vorherigen
Code, mit der `split()`-Funktion.
</Example>
